...
def edit_teacher(request, teacher_username):
    ...
    available_class_rooms = Class.objects.exclude(
        class_name__in=ClassRoom.objects.values_list("class_name", flat=True)
    )
...
def teacher_detail(request, teacher_username):
    ...
    context = {
        "teacher": teacher,
        "home_room": (
            ClassRoom.objects.get(room_teacher=teacher).class_name
...
def delete_teacher(request, teacher_username):
    if request.method == "POST":
        ...
        AdminAction.objects.create(
            admin=request.user, action=f"Deleted Teacher ({teacher.username})"
        )
        teacher.delete()
    ...
    return redirect(
        reverse("teacher_detail_url", kwargs={"teacher_username": teacher_username})
    )
...
def add_teacher(request):
    available_class_rooms = Class.objects.exclude(
        class_name__in=ClassRoom.objects.values("class_name")
    )
    context = {
    ...
    if request.method == "POST":
        try:
            ...
            if ClassRoom.objects.filter(
                class_name__class_name=home_room_class
            ).exists():
                messages.error(
                    request,
                    f"There is already a home room teacher for {home_room_class}.",
                ...
                messages.warning(
                    request, f"This teacher doesn't have a homeroom class."
                )
            ...
            try:
                teacher_profile = UserProfile.objects.create(
                    user=teacher, user_pic=profile_pic, password=password
                )
            except Exception as e:
            ...
            admin_action = AdminAction(
                admin=request.user, action=f"Added Teacher ({teacher.first_name})"
            )
            admin_action.save()
...
def add_teachers(request):
    ...
    if request.method == "POST":
        ...
        try:
            ...
            df = pd.read_excel(
                file, dtype={"phone number": str, "home room class": str}
            ).fillna("")
            df.columns = [
        ...
        taken_home_rooms = set(
            ClassRoom.objects.values_list("class_name__class_name", flat=True)
        )
        ...
        for idx, row in df.iterrows():
            ...
            if home_class:
                if home_class in taken_home_rooms:
                    ...
                    row_errors.append(
                        f"Line {line}: Invalid class format '{home_class}'"
                    )
            ...
            for name_field in ["first name", "middle name", "last name"]:
                ...
                if not re.match(r"^[A-Za-z\-\s]+$", name):
                    row_errors.append(
                        f"Line {line}: Invalid characters in {name_field} '{name}'"
                    )
        ...
        try:
            with transaction.atomic():
                ...
                for row_data, home_class, age, gender, email in to_create:
                    ...
                    try:
                        user = UserProfile.objects.create(
                            user=user, user_pic=default_avatar_path, password=pwd
                        )
                    except Exception as e:
                ...
                AdminAction.objects.create(
                    admin=request.user, action=f"Bulk added {len(new_users)} teachers"
                )

                messages.success(
                    request, f"Successfully added {len(new_users)} teachers"
                )
                return redirect("teachers_mang_url")
